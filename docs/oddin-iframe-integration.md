# Oddin esports (Bifrost) — operator checklist

**Bifrost** is Oddin’s name for their **browser iframe / JS client** — it is part of Oddin’s product, not a separate vendor or third-party stack. This doc uses “Bifrost” when referring to Oddin’s script, URLs, and `buildBifrost` API.
...
Give Oddin a consistent picture of your public surfaces so they can allowlist origins, configure their Bifrost client, and call your **operator (server-to-server) wallet API**.
...
| **Oddin iframe (Bifrost)** | **Brand token**, **base URL**, **script URL** (integration vs production; Oddin hosts these on `bifrost.*.oddin.gg`) | Player SPA: `VITE_ODDIN_BRAND_TOKEN`, `VITE_ODDIN_BASE_URL`, `VITE_ODDIN_SCRIPT_URL`. Core mirrors public URLs for diagnostics: `ODDIN_PUBLIC_BASE_URL`, `ODDIN_PUBLIC_SCRIPT_URL`. |
...
## How this repo embeds the Oddin iframe (Bifrost)

1. **Feature flag & public config**: **`/casino/sports`** mounts the **Oddin iframe** when the **merged** public config validates (from **player** `VITE_ODDIN_*` and/or **`GET /v1/sportsbook/oddin/public-config`** when core has `ODDIN_ENABLED` + `ODDIN_BRAND_TOKEN` + `ODDIN_PUBLIC_BASE_URL` + `ODDIN_PUBLIC_SCRIPT_URL`). You can configure Oddin **only on core** (no `VITE_ODDIN_ENABLED`) or **only on the player**, or **both** (Vite values win when set; gaps are filled from the API).
...
4. **Session token**: Authenticated players obtain an opaque token via **`POST /v1/sportsbook/oddin/session-token`** (requires `ODDIN_ENABLED` on core). The token is passed into **Oddin’s Bifrost client** as `token`; **`brandToken`**, **`baseUrl`**, language, currency, theme, and `darkMode` come from public env (see `readOddinPublicConfig()` / `useOddinBifrost`).
...
**Token validation is live**: Oddin's `Authenticator.ParseRequest` resolves a player from an **Oddin session token**. Until **debit/credit** are wired to the casino ledger (still stubs returning `errorCode: 4`), bet placement will fail server-side even though sign-in/balance refresh succeed; implement ledger debits/credits per Oddin's contract and replace the stubs in `services/core/internal/oddin/handler_operator.go`.
...
- Prefer **HTTPS Oddin iframe URLs** in production (`ODDIN_PUBLIC_*` / `VITE_ODDIN_*`): `ODDIN_ENV=production`, `ODDIN_PUBLIC_BASE_URL=https://bifrost.oddin.gg`, `ODDIN_PUBLIC_SCRIPT_URL=https://bifrost.oddin.gg/script.js` (exact URLs from Oddin).
- Give Oddin your **player origin** (`https://…vercel.app` or custom domain) if they allowlist **iframe** hosts per operator.
...
**Admin Console → Oddin (esports)** (`OddinIntegrationPage`) calls **`GET /v1/admin/integrations/oddin`** for non-secret health counters (iframe events, operator callback errors in audit). Use it together with Oddin’s dashboard and this checklist when debugging integration.

## Handoff line for Oddin support

You can send something shaped like:

> Player app: `https://<player-host>/esports` (Oddin esports iframe / Bifrost).  

...
type AuthState = {
  ...
  login: (email: string, password: string) => Promise<{ ok: true } | { ok: false; error: ApiErr }>
  logout: () => Promise<void>
...
export function AdminAuthProvider({ children }: { children: ReactNode }) {
  ...
  const login = useCallback(
    async (
      e: string,
      password: string,
    ): Promise<{ ok: true } | { ok: false; error: ApiErr }> => {
      try {
        ...
        if (!res.ok) {
          const parsed = await readApiError(res.clone())
          ...
          return {
            ...
            error: {
              ...
              status: res.status,
              message: unreachable
                ? 'Backend unreachable. From the repo root run npm run dev:api (port 8080) and ensure Postgres is running (e.g. npm run compose:up).'

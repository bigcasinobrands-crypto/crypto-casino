...
function classifyDisplayType(entryType: string, amountMinor: number): TxDisplayType {
  ...
  if (entryType === 'deposit.credit') return 'received'
  if (entryType === 'deposit.reversal') return 'sent'
  if (entryType === 'withdrawal.pending.settled') return 'withdrawal'
  if (entryType === 'withdrawal.compensation' || entryType === 'withdrawal.compensation.cash') return 'refund'
  if (entryType === 'game.credit' || entryType === 'game.win') return 'received'
  if (entryType === 'game.debit' || entryType === 'game.bet') return 'sent'
  if (entryType === 'game.rollback') return 'refund'
  if (entryType === 'sportsbook.credit') return 'received'
  if (entryType === 'sportsbook.debit') return 'sent'
  if (entryType === 'sportsbook.rollback') return 'refund'

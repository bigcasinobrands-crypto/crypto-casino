...
func (h *Handler) ListStaffUsers(w http.ResponseWriter, r *http.Request) {
	rows, err := h.Pool.Query(r.Context(), `
		SELECT id::text, email, role, created_at FROM staff_users ORDER BY created_at ASC
	`)
	if err != nil {
	...
	for rows.Next() {
		...
		var ct time.Time
		if err := rows.Scan(&id, &email, &role, &ct); err != nil {
			continue
		}
		list = append(list, map[string]any{
			"id": id, "email": email, "role": role, "created_at": ct.UTC().Format(time.RFC3339),
		})
	}
